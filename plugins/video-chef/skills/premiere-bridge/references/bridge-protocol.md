...
The broker uses protocol `1.0`. The bundled connector advertises `ping` and `snapshot_active_sequence`, plus `mutation_enabled: false`. The broker will reject undeclared capabilities, arbitrary operations, and any write request.

## Endpoints

- `POST /v1/connector/register`: connector version, Premiere version, and capability negotiation.
...
- `GET /v1/status`: protocol, registered connector, and mutation state.
...
- `503`: no UXP connector is registered. Start the broker, launch the panel in Premiere, and connect.
